...
def get_args_parser():
    ...
    parser.add_argument('--vis_freq', default=500, type=str)
...
def voxel_grid_visualization(input, output_path):
    ...
    imageio.mimsave(output_path, my_images, fps=10)



def point_cloud_visualization(input, output_path):
...
def mesh_visualization(input, output_path):
...
def evaluate(predictions, mesh_gt, thresholds, args):
    if args.type == "vox":
        ...
        #Pratik
        print('Printing predictions...', predictions.shape)
        print(predictions)
...
def evaluate_model(args):
    ...
    for step in range(start_iter, max_iter):
        ...
        if (step % args.vis_freq) == 0:
            if args.type == "vox":
                voxel_grid_visualization(predictions, output_path=f'output/vox_{step}.gif')
                voxel_gt = feed_dict['voxels'].to(args.device)
                voxel_grid_visualization(voxel_gt, output_path=f'output/gt_vox_{step}.gif')

            elif args.type == "point":
                point_cloud_visualization(predictions, output_path=f'output/pc_{step}.gif')
                point_cloud_gt = sample_points_from_meshes(mesh_gt, args.n_points).to(args.device)
                point_cloud_visualization(point_cloud_gt, output_path=f'output/gt_pc_{step}.gif')

            elif args.type == "mesh":
                mesh_visualization(predictions, output_path=f'output/mesh_{step}.gif')
                mesh_visualization(mesh_gt, output_path=f'output/gt_mesh_{step}.gif')    
